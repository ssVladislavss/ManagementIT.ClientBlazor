...
{
    //public class RegisterEmployeeFluentValidator : AbstractValidator<RegisterEmployeeModel>
    //{
    //    public RegisterEmployeeFluentValidator()
    //    {
    ...
    //            //.Must(x => IsValidPassword(x)).WithMessage("Пароль не валиден");
    ...
    //    {
    //        var result = await ValidateAsync(ValidationContext<RegisterEmployeeModel>
    //            .CreateWithOptions((RegisterEmployeeModel)model, x => x.IncludeProperties(propertyName)));
    //        if (result.IsValid)
    ...
    //        return !Regex.IsMatch(password, (@"^[а-яА-Яa-zA-Z0-9ёЁ!№;%:?*() _+=\-<>.,/\\'`@#$%^&{}\[\]|""~ ]{0,50}$"));
    //    }
